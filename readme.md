...
Создание веток в Git - очень увлекательная процедура. Для начала научимся в принципе команде, которая позволит нам увидеть все существующие ветки на данный момент. Такая команда выполняется запуском **git branch**. 
